...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($field_errors['title'] || $field_errors['description']) {
        // Individual field errors, no global errors
    } else {
        $stmt = $conn->prepare("UPDATE why_aone_hub SET why_title = ?, why_description = ? WHERE why_id = ?");
        if ($stmt) {
            $stmt->bind_param("ssi", $ptitle, $pdesc, $id);
...
<head>
    <meta charset="UTF-8">
    <title>Edit Why Aone Hub Reason</title>
...
<div class="dashboard-main">
    <h2 class="internal-header">Edit Why Aone Hub Reason</h2>
    <div class="coupon-form-wrap">
        ...
        <form method="post" id="whyAoneHubForm" autocomplete="off" novalidate>
            <div class="form-group">
                ...
                <input type="text" name="title" id="title" maxlength="128" value="<?= esc($_POST['title'] ?? $title) ?>" autocomplete="off" />
                <span class="form-error" id="err_title"><?php if (!empty($field_errors['title'])) echo $field_errors['title']; ?></span>
            </div>
            <div class="form-group">
                <label for="description">Description <span style="color:red">*</span></label>
                <textarea name="description" id="description" rows="3" maxlength="500"><?= esc($_POST['description'] ?? $description) ?></textarea>
